...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(ComplexFood.schema, ComplexIngredient.schema, ConversionFactor.schema, FoodGroup.schema, FoodName.schema, FoodSource.schema, Meal.schema, MealEntry.schema, MeasureName.schema, NutrientAmount.schema, NutrientName.schema, NutrientSource.schema, Profile.schema, Recipe.schema, RecipeIngredient.schema, ReferenceEntry.schema, ReferenceMap.schema, RefuseAmount.schema, RefuseName.schema, Session.schema, User.schema, YieldAmount.schema, YieldName.schema).reduceLeft(_ ++ _)
  ...
  case class ComplexFoodRow(recipeId: java.util.UUID, amountGrams: scala.math.BigDecimal, amountMilliLitres: Option[scala.math.BigDecimal] = None, userId: java.util.UUID)
  ...
  implicit def GetResultComplexFoodRow(implicit e0: GR[java.util.UUID], e1: GR[scala.math.BigDecimal], e2: GR[Option[scala.math.BigDecimal]]): GR[ComplexFoodRow] = GR{
    prs => import prs._
    ComplexFoodRow.tupled((<<[java.util.UUID], <<[scala.math.BigDecimal], <<?[scala.math.BigDecimal], <<[java.util.UUID]))
  }
  ...
  class ComplexFood(_tableTag: Tag) extends profile.api.Table[ComplexFoodRow](_tableTag, "complex_food") {
    ...
    def ? = ((Rep.Some(recipeId), Rep.Some(amountGrams), amountMilliLitres, Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> ComplexFoodRow.tupled((_1.get, _2.get, _3, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Primary key of ComplexFood (database name complex_food_pk) */
    val pk = primaryKey("complex_food_pk", (userId, recipeId))

    /** Foreign key referencing Recipe (database name complex_food_user_id_recipe_id_fk) */
    lazy val recipeFk = foreignKey("complex_food_user_id_recipe_id_fk", (userId, recipeId), Recipe)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing User (database name complex_food_user_id_fk) */
    lazy val userFk = foreignKey("complex_food_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
   ...
   *  @param scalingMode Database column scaling_mode SqlType(text)
   *  @param userId Database column user_id SqlType(uuid) */
  case class ComplexIngredientRow(recipeId: java.util.UUID, complexFoodId: java.util.UUID, factor: scala.math.BigDecimal, scalingMode: String, userId: java.util.UUID)
  ...
  implicit def GetResultComplexIngredientRow(implicit e0: GR[java.util.UUID], e1: GR[scala.math.BigDecimal], e2: GR[String]): GR[ComplexIngredientRow] = GR{
    prs => import prs._
    ComplexIngredientRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[scala.math.BigDecimal], <<[String], <<[java.util.UUID]))
  }
  ...
  class ComplexIngredient(_tableTag: Tag) extends profile.api.Table[ComplexIngredientRow](_tableTag, "complex_ingredient") {
    def * = (recipeId, complexFoodId, factor, scalingMode, userId) <> (ComplexIngredientRow.tupled, ComplexIngredientRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(recipeId), Rep.Some(complexFoodId), Rep.Some(factor), Rep.Some(scalingMode), Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> ComplexIngredientRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val pk = primaryKey("complex_ingredient_pk", (userId, recipeId, complexFoodId))

    /** Foreign key referencing ComplexFood (database name complex_ingredient_user_id_complex_food_id_fk) */
    lazy val complexFoodFk = foreignKey("complex_ingredient_user_id_complex_food_id_fk", (userId, complexFoodId), ComplexFood)(r => (r.userId, r.recipeId), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing Recipe (database name complex_ingredient_user_id_recipe_id_fk) */
    lazy val recipeFk = foreignKey("complex_ingredient_user_id_recipe_id_fk", (userId, recipeId), Recipe)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing User (database name complex_ingredient_user_id_fk) */
    lazy val userFk = foreignKey("complex_ingredient_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
   ...
   *  @param profileId Database column profile_id SqlType(uuid) */
  case class MealRow(id: java.util.UUID, userId: java.util.UUID, consumedOnDate: java.sql.Date, consumedOnTime: Option[java.sql.Time] = None, name: Option[String] = None, profileId: java.util.UUID)
  ...
  implicit def GetResultMealRow(implicit e0: GR[java.util.UUID], e1: GR[java.sql.Date], e2: GR[Option[java.sql.Time]], e3: GR[Option[String]]): GR[MealRow] = GR{
    prs => import prs._
    MealRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[java.sql.Date], <<?[java.sql.Time], <<?[String], <<[java.util.UUID]))
  }
  ...
  class Meal(_tableTag: Tag) extends profile.api.Table[MealRow](_tableTag, "meal") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(userId), Rep.Some(consumedOnDate), consumedOnTime, name, Rep.Some(profileId))).shaped.<>({r=>import r._; _1.map(_=> MealRow.tupled((_1.get, _2.get, _3.get, _4, _5, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column profile_id SqlType(uuid) */
    val profileId: Rep[java.util.UUID] = column[java.util.UUID]("profile_id")

    /** Primary key of Meal (database name meal_pk) */
    val pk = primaryKey("meal_pk", (userId, profileId, id))

    /** Foreign key referencing Profile (database name meal_user_id_profile_id_fk) */
    lazy val profileFk = foreignKey("meal_user_id_profile_id_fk", (userId, profileId), Profile)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
   ...
   *  @param userId Database column user_id SqlType(uuid)
   *  @param profileId Database column profile_id SqlType(uuid) */
  case class MealEntryRow(id: java.util.UUID, mealId: java.util.UUID, recipeId: java.util.UUID, numberOfServings: scala.math.BigDecimal, userId: java.util.UUID, profileId: java.util.UUID)
  ...
  implicit def GetResultMealEntryRow(implicit e0: GR[java.util.UUID], e1: GR[scala.math.BigDecimal]): GR[MealEntryRow] = GR{
    prs => import prs._
    MealEntryRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[java.util.UUID], <<[scala.math.BigDecimal], <<[java.util.UUID], <<[java.util.UUID]))
  }
  ...
  class MealEntry(_tableTag: Tag) extends profile.api.Table[MealEntryRow](_tableTag, "meal_entry") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(mealId), Rep.Some(recipeId), Rep.Some(numberOfServings), Rep.Some(userId), Rep.Some(profileId))).shaped.<>({r=>import r._; _1.map(_=> MealEntryRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Primary key of MealEntry (database name meal_entry_pk) */
    val pk = primaryKey("meal_entry_pk", (userId, profileId, mealId, id))

    /** Foreign key referencing Meal (database name meal_entry_user_id_profile_id_meal_id_fk) */
    lazy val mealFk = foreignKey("meal_entry_user_id_profile_id_meal_id_fk", (userId, profileId, mealId), Meal)(r => (r.userId, r.profileId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing Profile (database name meal_entry_user_id_profile_id_fk) */
    lazy val profileFk = foreignKey("meal_entry_user_id_profile_id_fk", (userId, profileId), Profile)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing Recipe (database name meal_entry_user_id_recipe_id_fk) */
    lazy val recipeFk = foreignKey("meal_entry_user_id_recipe_id_fk", (userId, recipeId), Recipe)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing User (database name meal_entry_user_id_fk) */
    lazy val userFk = foreignKey("meal_entry_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
  ...
  case class ProfileRow(id: java.util.UUID, userId: java.util.UUID, name: String)
  /** GetResult implicit for fetching ProfileRow objects using plain SQL queries */
  implicit def GetResultProfileRow(implicit e0: GR[java.util.UUID], e1: GR[String]): GR[ProfileRow] = GR{
    prs => import prs._
  ...
  class Profile(_tableTag: Tag) extends profile.api.Table[ProfileRow](_tableTag, "profile") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(userId), Rep.Some(name))).shaped.<>({r=>import r._; _1.map(_=> ProfileRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Primary key of Profile (database name profile_pk) */
    val pk = primaryKey("profile_pk", (userId, id))

    /** Foreign key referencing User (database name profile_user_id_fk) */
    lazy val userFk = foreignKey("profile_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)

    /** Uniqueness Index over (name) (database name profile_name_key) */
    val index1 = index("profile_name_key", name, unique=true)
  }
  ...
  class Recipe(_tableTag: Tag) extends profile.api.Table[RecipeRow](_tableTag, "recipe") {
    ...
    /** Primary key of Recipe (database name recipe_pk) */
    val pk = primaryKey("recipe_pk", (userId, id))
   ...
   *  @param factor Database column factor SqlType(numeric)
   *  @param userId Database column user_id SqlType(uuid) */
  case class RecipeIngredientRow(id: java.util.UUID, recipeId: java.util.UUID, foodNameId: Int, measureId: Option[Int] = None, factor: scala.math.BigDecimal, userId: java.util.UUID)
  ...
  implicit def GetResultRecipeIngredientRow(implicit e0: GR[java.util.UUID], e1: GR[Int], e2: GR[Option[Int]], e3: GR[scala.math.BigDecimal]): GR[RecipeIngredientRow] = GR{
    prs => import prs._
    RecipeIngredientRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[Int], <<?[Int], <<[scala.math.BigDecimal], <<[java.util.UUID]))
  }
  ...
  class RecipeIngredient(_tableTag: Tag) extends profile.api.Table[RecipeIngredientRow](_tableTag, "recipe_ingredient") {
    def * = (id, recipeId, foodNameId, measureId, factor, userId) <> (RecipeIngredientRow.tupled, RecipeIngredientRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(recipeId), Rep.Some(foodNameId), measureId, Rep.Some(factor), Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> RecipeIngredientRow.tupled((_1.get, _2.get, _3.get, _4, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Primary key of RecipeIngredient (database name recipe_ingredient_pk) */
    val pk = primaryKey("recipe_ingredient_pk", (userId, recipeId, id))
    ...
    /** Foreign key referencing Recipe (database name recipe_ingredient_user_id_recipe_id_fk) */
    lazy val recipeFk = foreignKey("recipe_ingredient_user_id_recipe_id_fk", (userId, recipeId), Recipe)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing User (database name recipe_ingredient_user_id_fk) */
    lazy val userFk = foreignKey("recipe_ingredient_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
   ...
   *  @param userId Database column user_id SqlType(uuid) */
  case class ReferenceEntryRow(referenceMapId: java.util.UUID, nutrientCode: Int, amount: scala.math.BigDecimal, userId: java.util.UUID)
  ...
  class ReferenceEntry(_tableTag: Tag) extends profile.api.Table[ReferenceEntryRow](_tableTag, "reference_entry") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(referenceMapId), Rep.Some(nutrientCode), Rep.Some(amount), Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> ReferenceEntryRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val pk = primaryKey("reference_entry_pk", (userId, referenceMapId, nutrientCode))
    ...
    /** Foreign key referencing ReferenceMap (database name reference_entry_user_id_reference_map_id_fk) */
    lazy val referenceMapFk = foreignKey("reference_entry_user_id_reference_map_id_fk", (userId, referenceMapId), ReferenceMap)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing User (database name reference_entry_user_id_fk) */
    lazy val userFk = foreignKey("reference_entry_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
  ...
  class ReferenceMap(_tableTag: Tag) extends profile.api.Table[ReferenceMapRow](_tableTag, "reference_map") {
    ...
    /** Primary key of ReferenceMap (database name reference_map_pk) */
    val pk = primaryKey("reference_map_pk", (userId, id))
  ...
  implicit def GetResultSessionRow(implicit e0: GR[java.util.UUID], e1: GR[java.sql.Date]): GR[SessionRow] = GR{
    prs => import prs._
