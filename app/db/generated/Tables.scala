...
trait Tables {
  ...
  class Meal(_tableTag: Tag) extends profile.api.Table[MealRow](_tableTag, "meal") {
    ...
    val pk = primaryKey("meal_pk", (userId, id))
   ...
   *  @param userId Database column user_id SqlType(uuid) */
  case class MealEntryRow(id: java.util.UUID, mealId: java.util.UUID, recipeId: java.util.UUID, numberOfServings: scala.math.BigDecimal, userId: java.util.UUID)
  ...
  implicit def GetResultMealEntryRow(implicit e0: GR[java.util.UUID], e1: GR[scala.math.BigDecimal]): GR[MealEntryRow] = GR{
    prs => import prs._
    MealEntryRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[java.util.UUID], <<[scala.math.BigDecimal], <<[java.util.UUID]))
  }
  ...
  class MealEntry(_tableTag: Tag) extends profile.api.Table[MealEntryRow](_tableTag, "meal_entry") {
    def * = (id, mealId, recipeId, numberOfServings, userId) <> (MealEntryRow.tupled, MealEntryRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(mealId), Rep.Some(recipeId), Rep.Some(numberOfServings), Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> MealEntryRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val pk = primaryKey("meal_entry_pk", (userId, mealId, id))

    /** Foreign key referencing Meal (database name meal_entry_user_id_meal_id_fk) */
    lazy val mealFk = foreignKey("meal_entry_user_id_meal_id_fk", (userId, mealId), Meal)(r => (r.userId, r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
